...
*/
function is31Flavors(arrayChk) {
  let arrLngth = arrayChk.length;
  if (arrLngth === 31) {
    return true;
  } else {
    return false;
...
function addFlavor(origArray, newString) {
  origArray.unshift(newString);
  return origArray;
}
...
function getFlavorByIndex(flavArr, indexNum) {
  let newArr = flavArr.slice(indexNum, indexNum + 1);
  return newArr[0];
}
console.log("task 5", getFlavorByIndex(originalFlavors, 2));

/*🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 6: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
As corporate wants to add more and more flavors to their lineup, they've realized that they need to remove flavors based on flavor name, as opposed to 
just arbitrarily removing the first or last flavor. Your task is to get an index by flavor name, and remove that single flavor from the array.  
...
Use the removeFlavorByName function below to do the following:
  ...
  For example: running removeFlavorByName(originalFlavors, "Rocky Road") would return an array with the a length of 30 because Rocky Road would have 
  been removed. 

  HINT: You can use .splice() for this
*/

function removeFlavorByName(masterArray, delFlav) {
  for (let i = 0; i < masterArray.length; i++) {
    if (masterArray[i] === delFlav) {
...
Your task is to write a function that checks every item in the array for a string and returns a new array called filteredArray with only the values that 
contain the received string. This would allow you to be able to filter for "Vanilla", "Sherbet", "Lemon" etc. when different holidays roll around by 
...
Use the filterByWord function below to do the following:
  ...
  For example: filterByWord(originalFlavors, "Chocolate") should return ["Chocolate", "Chocolate Almond", "Chocolate Chip", "Chocolate Fudge", 
...
function filterByWord(mainArray, filterString) {
  let newArray = [];
  for (let i = 0; i < mainArray.length; i++) {
    if (mainArray[i].includes(filterString)) {
...
/* STRETCH 1: Write a function that returns the average number of words in an array. You should be able to use this function for any array, but can 
test with originalFlavors.
...
function getAverageWordLength(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    wordsCounter += array[i].split(" ").length;
  }
  return Math.round(wordsCounter / array.length);
}
console.log(
  "stretch 1:",
...
function getRandomFlavors(array1, array2, array3, array4) {
  let RandomFlavor = [];
  for (let i = 0; i < 8; i++) {
    RandomFlavor.push(array1[Math.floor(Math.random() * array1.length)]);
    RandomFlavor.push(array2[Math.floor(Math.random() * array2.length)]);
    RandomFlavor.push(array3[Math.floor(Math.random() * array3.length)]);
    RandomFlavor.push(array4[Math.floor(Math.random() * array4.length)]);
  }
  RandomFlavor.pop();
  console.log("Stretch 2: number of random flavors= ", RandomFlavor.length);
  return RandomFlavor;
}
console.log(
  getRandomFlavors(
